...
if not "%TOOLSET%" == "%TOOLSET:msvc-=%" (
  if not "%ADDRESS_MODEL%" == "64" (
    call :XCOPY_FILE "%%BUILD_ROOT%%\lib\%%VARIANT_VALUE%%" "gmock%%MSVC_LIB_SUFFIX%%.lib"      "%%BUILD_ROOT%%\lib\x86\%%VARIANT_VALUE%%" /Y /D /H || exit /b
    call :XCOPY_FILE "%%BUILD_ROOT%%\lib\%%VARIANT_VALUE%%" "gmock_main%%MSVC_LIB_SUFFIX%%.lib" "%%BUILD_ROOT%%\lib\x86\%%VARIANT_VALUE%%" /Y /D /H || exit /b
  ) else (
    call :XCOPY_FILE "%%BUILD_ROOT%%\lib\%%VARIANT_VALUE%%" "gmock%%MSVC_LIB_SUFFIX%%.lib"      "%%BUILD_ROOT%%\lib\x64\%%VARIANT_VALUE%%" /Y /D /H || exit /b
    call :XCOPY_FILE "%%BUILD_ROOT%%\lib\%%VARIANT_VALUE%%" "gmock_main%%MSVC_LIB_SUFFIX%%.lib" "%%BUILD_ROOT%%\lib\x64\%%VARIANT_VALUE%%" /Y /D /H || exit /b
  )
)
